$(document).ready(function () {
    ...
    // Uniform
    var checkBox = $(
      "input[type=checkbox]:not(.switchery), input[type=radio]:not(.no-uniform)"
    );
    if (checkBox.size() > 0) {
    ...
    if ($(".chat").length) {
      ...
      $(".chat-write form input").keypress(function (e) {
        if (e.which == 13 && !$(this).val().length == 0) {
          $(
            '<div class="chat-item chat-item-right"><div class="chat-message">' +
              $(this).val() +
              "</div></div>"
          ).insertAfter(".chat .chat-item:last-child");
          $(this).val("");
        } else if (e.which == 13) {
          return;
        }
